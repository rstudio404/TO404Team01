...
```
Here we run into the pattern of having a large difference in ride numbers across genders (0=unknown, 1=male, 2=female), the male shows drastically larger ridership than the female, together with steeper up and down slope. This could be related to the gender structure of population, but also could be related to men's more physical strength. More data are needed for further analysis. Anyway, in general, we can still see that the same curve of having the most number of rides at around 75 degrees is present across all gender categories. 
...
```
The same problem as before is here, where the number of total bikes is very different between customers and subscribers. It is obvious that subscribers contribute more to the number of ridership, and we can again see that the general curve is the same (peak at around 75 degrees). Interestingly, customers ride more at weekends than weekdays while subscribers shows the opposite. This indicates that most subscribers use citibikes as commuting vehicles, and most customers use them for leisure.
...
baseplot_Week + geom_point(size = 3, alpha = 0.5) + geom_smooth() + facet_wrap(~ dayid)
```
During the weekdays, average speed declines slightly with temperature. During the weekend, average speed is at a maximum when it is around 40 degrees, and a minimum when it is at 65 degrees. This strange pattern is hard to explain based on exsiting data, but we can speculate that maybe when it's too cold or hot, people prefer to ride faster to arrive their destination as soon as possible. While as the temp is closing to 74 degrees, people feel more comfortable and prefer to enjoy the riding. 
...
baseplot_userT <- ggplot(data = combined_userT, aes(x = AWND, y = count, colour = dayid))
baseplot_userT + geom_point(size = 3, alpha = .5) + geom_smooth() + facet_wrap(~ usertype)
baseplot_age <- ggplot(data = combined_age, aes(x = AWND, y = count, colour = dayid))
baseplot_age + geom_point(size = 3, alpha = .5) + geom_smooth() + facet_wrap(~ agegroup)
```
From the first graph, it is evident that ridership on weekday is slightly higher than that of weekend, while the number of rides fluctuates smaller than that of weekend. On weekends, the first peak appears around AWND = 2.5, with the second one around AWND = 5.2. This implies people's preference of riding outside at weekend to some degree.
...
Graphs based on gender and age groups display the same trend. In detail, the differences between weekday and weekend for customers and subscribers and among different age groups are identical with that in temperature part. 
...
In the second graph, the overall trend among genders is similar to that of temperature part. And men shows stronger physical abilities to maintain higher speed.

Moving to the third graph, subscribers' speed is higher drastically than that of customers. From the website of our dataset resource, we learned that customer is defined as 24-hour pass or 3-day pass user. We think these people are more likely to be visitors which tend to pursue relax and comfort during their trips. On the contrary, we know that most subscribers are riding to work from our previous analysis. Thus, it's easy to understand that their faster speed is to save time. 
...
baseplot_Week + geom_point(size = 3, alpha = .5) + geom_smooth()
baseplot_gender <- ggplot(data = combined_gender, aes(x = PRCP, y = count, colour = dayid))
baseplot_gender + geom_point(size = 3, alpha = .5) + geom_smooth() + facet_wrap(~ gender)
baseplot_userT <- ggplot(data = combined_userT, aes(x = PRCP, y = count, colour = dayid))
baseplot_userT + geom_point(size = 3, alpha = .5) + geom_smooth() + facet_wrap(~ usertype)
baseplot_age <- ggplot(data = combined_age, aes(x = PRCP, y = count, colour = dayid))
baseplot_age + geom_point(size = 3, alpha = .5) + geom_smooth() + facet_wrap(~ agegroup)
```
Except for the explanation already included above, there may be some new insights from the impact of precipitation on ridership.
